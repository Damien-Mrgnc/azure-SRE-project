...
async function getConfig() {
    ...
    try {
        if (redisClient.isReady) {
            const cached = await redisClient.get('appConfig');
            if (cached) {
                logger.info('Config retrieved from Redis cache');
                return JSON.parse(cached);
            }
        }
    } catch (e) {
        logger.warn('Redis cache read failed, falling back to db', e);
    }
    ...
    // 2. Save result to Redis Cache (expires in 5 mins)
    try {
        if (redisClient.isReady) {
            await redisClient.set('appConfig', JSON.stringify(result), { EX: 300 });
        }
    } catch (e) {
        logger.warn('Redis cache set failed', e);
    }
...
async function updateConfig(newConfig, changedBy = 'system', correlationId) {
    ...
    if (operations.length > 0) {
        ...
        try {
            if (redisClient.isReady) {
                await redisClient.del('appConfig');
                logger.info('Cache invalidated after config update');
            }
        } catch (e) {
